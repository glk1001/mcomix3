...
class OpenWithCommand(object):
    ...
    def execute(self, window):
        ...
        and arguments. """
        if self.is_disabled_for_archives() and window.filehandler.archive_type is not None:
        ...
        try:
            ...
            text = f'Could not run command {self.get_label()}: {e}'
...
class OpenWithEditor(Gtk.Dialog):
    ...
    def get_commands(self):
        ...
        commands = []
        while itr:
            label, command, cwd, disabled_for_archives = model.get(itr, 0, 1, 2, 3)
    ...
    def get_command(self):
        ...
        model, itr = self._command_tree.get_selection().get_selected()
        if itr and model.iter_is_valid(itr):
            command = OpenWithCommand(*model.get(itr, 0, 1, 2, 3))
            return command
        else:
            return None
    ...
    def _layout(self):
        ...
        hints_expander = Gtk.Expander.new('Hints of variables in command')
        ...
        hints_all = [(
                ('{image}', 'Full path of the currently opened image file'),
                ('{imagebase}', 'Base name of {}').format('"{image}"'),
                ('{imagedir}', 'Full directory name of {}').format('"{image}"'),
                ('{imagedirbase}', 'Base name of {}').format('"{imagedir}"'),
                ('{container}', 'Full path of the currently opened directory or archive'),
                ('{containerbase}', 'Base name of {}').format('"{container}"'),
                ('{containerdir}', 'Full directory name of {}').format('"{container}"'),
                ('{containerdirbase}', 'Base name of {}').format('"{containerdir}"'),
        ), (
                ('{archive}', 'Full path of the currently opened archive'),
                ('{archivebase}', 'Base name of {}').format('"{archive}"'),
                ('{archivedir}', 'Full directory name of {}').format('"{archive}"'),
                ('{archivedirbase}', 'Base name of {}').format('"{archivedir}"'),
                ('{{', '{'),
                ('}}', '}'),
        ...
        linklabel = Gtk.Label()
        linklabel.set_markup('Please refer to the <a href="https://sourceforge.net/p/mcomix/wiki/External_Commands">'
                             + ' external command documentation</a> for a list of usable variables and other hints.')
    ...
    def _text_changed(self, renderer, path, new_text, column):
        ...
        def delayed_set_value():
            old_value = model.get_value(itr, column)
            model.set_value(itr, column, new_text)
    ...
    def _value_changed(self, renderer, path, column):
        ...
        def delayed_set_value():
            ...
            model.set_value(itr, column, value)
            self._changed = True
