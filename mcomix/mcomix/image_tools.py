...
def get_most_common_edge_color(pixbufs, edge=2):
    ...
    def get_edge_pixbuf(pixbuf, side, edg):
        ...
        edg = min(edg, width, height)

        subpix = GdkPixbuf.Pixbuf.new(GdkPixbuf.Colorspace.RGB,
                                      pixbuf.get_has_alpha(), 8, edg, height)
        if side == 'left':
            pixbuf.copy_area(0, 0, edg, height, subpix, 0, 0)
        elif side == 'right':
            pixbuf.copy_area(width - edg, 0, edg, height, subpix, 0, 0)
        elif side == 'top':
            pixbuf.copy_area(0, 0, width, edg, subpix, 0, 0)
        elif side == 'bottom':
            pixbuf.copy_area(0, height - edg, width, edg, subpix, 0, 0)
        else:
