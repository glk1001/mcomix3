...
class Box(object):
    ...
    def align_center(boxes, axis, fix, orientation):
        ...
        @param boxes: the boxes to center.
    ...
    def distribute(boxes, axis, fix, spacing=0):
        ...
        if len(boxes) == 0:
            return []
        result: List[Union[None, Box]] = [None] * len(boxes)
    ...
    def intersect(box_a, box_b):  # TODO test! docs!
        a_pos = box_a.get_position()
        b_pos = box_b.get_position()
        a_size = box_a.get_size()
        b_size = box_b.get_size()
        res_pos = [0] * len(a_pos)
        res_size = [0] * len(a_size)
