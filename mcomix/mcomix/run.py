...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
            usage=f'{os.path.basename(sys.argv[0])} [OPTION...] [PATH...]',
            description='View images and comic book archives.',
            add_help=False)
    parser.add_argument('--help', action='help',
                        help='Show this help and exit.')
    parser.add_argument('path', type=str, action='store', nargs='*', default='', help=argparse.SUPPRESS)

    parser.add_argument('-s', '--slideshow', dest='slideshow', action='store_true',
                        help='Start the application in slideshow mode.')
    parser.add_argument('-l', '--library', dest='library', action='store_true', help='Show the library on startup.')
    ...
    debugopts.add_argument('-W', dest='loglevel', action='store',
                           ...
                           metavar="[ {' | '.join(log.levels.keys())} ]",
                           help='Sets the desired output log level.')
    # This supresses an error when MComix is used with cProfile
    debugopts.add_argument('-o', dest='output', action='store', default='', help=argparse.SUPPRESS)
...
def run():
    ...
    try:
        ...
        if not pilver:
            ...
            log.warning('Please download latest version of Pillow from https://pypi.org/project/Pillow/')
