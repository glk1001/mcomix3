...
def get_archive_info(path):
    ...
    with get_recursive_archive_handler(path, typ=mime, prefix='mcomix_archive_info.') as archive:
        if archive is None:
            return None
...
def get_archive_handler(path, typ=None):
    ...
    """
    if typ is None:
        typ = archive_mime_type(path)
        if typ is None:
            return None

    handler = _get_handler(typ)
    if handler is None:
        return None
...
def get_recursive_archive_handler(path, typ=None, **kwargs):
    """ Same as <get_archive_handler> but the handler will transparently handle
    archives within archives.
    """
    archive = get_archive_handler(path, typ=typ)
    if archive is None:
        return None
