...
class _PreferencesDialog(Gtk.Dialog):
    ...
    def _init_appearance_tab(self):
        ...
        page.add_row(Gtk.Label(label='Language (needs restart):'), self._create_language_control())
        ...
        page.add_row(Gtk.Label(label='User theme'), self._create_pref_path_chooser('userstyle', default=None))
        ...
        page.add_row(self._create_pref_check_button(
                ...
                'By enabling this setting, the first page of a book will be used'
                + ' as application icon instead of the standard icon.'))

        page.add_row(Gtk.Label(label='Thumbnail size (in pixels):'),
                     self._create_pref_spinner('thumbnail size', 1, 20, 500, 1, 10, 0, None))

        page.add_row(
                Gtk.Label(label='Maximum number of thumbnail threads:'),
                self._create_pref_spinner(
                        'max thumbnail threads', 1, 0, constants.CPU_COUNT, 1, 4, 0,
                        'Set the maximum number of thumbnail generation threads (0 to use all available cores).'))
        ...
        page.add_row(self._create_pref_check_button(
                ...
                'Use a grey checkered background for transparent images.'
                + ' If this preference is unset, the background is plain white instead.'))
    ...
    def _init_onscrdisp_tab(self):
        ...
        page.add_row(Gtk.Label(label='Timeout:'),
                     self._create_pref_spinner('osd timeout', 1.0, 0.5, 30.0, 0.5, 2.0, 1,
                                               'Erase OSD after timeout, in seconds.'))

        page.add_row(Gtk.Label(label='Max font size:'),
                     self._create_pref_spinner('osd max font size', 1, 8, 60, 1, 10, 0,
                                               'Font size in OSD, hard limited from 8 to 60.'))

        page.add_row(Gtk.Label(label='Font color:'), self._create_color_button('osd color'))

        page.add_row(Gtk.Label(label='Background color:'), self._create_color_button('osd bg color'))
    ...
    def _init_behaviour_tab(self):
        ...
        page.add_row(self._create_pref_check_button(
                ...
                'flip with wheel',
                'Flip pages when scrolling "off the page" with the scroll wheel or with the arrow keys. It takes'
                + ' n consecutive "steps" with the scroll wheel or the arrow keys for the pages to be flipped.'))

        page.add_row(self._create_pref_check_button(
                'Automatically open the next archive',
                'auto open next archive',
                'Automatically open the next archive in the directory when flipping past the last page,'
                + ' or the previous archive when flipping past the first page.'))

        page.add_row(self._create_pref_check_button(
                'Automatically open next directory',
                'auto open next directory',
                'Automatically open the first file in the next sibling directory when flipping past'
                + ' the last page of the last file in a directory, or the previous directory when'
                + ' flipping past the first page of the first file.'))

        page.add_row(self._create_pref_check_button(
                'Open first file when navigating to previous archive',
                'open first file in prev archive',
                'Automatically open the first file of the previous archive when navigating to it,'
                + ' instead of opening the last file of the previous archive.'))

        page.add_row(self._create_pref_check_button(
                'Open first file when navigating to previous directory',
                'open first file in prev directory',
                'Automatically open the first file of the previous directory when navigating to it,'
                + ' instead of opening the last file of the previous directory.'))

        page.add_row(self._create_pref_check_button(
                'Dive into subdirectories (restart required)',
                'dive into subdir',
                'Dive into subdirectories when opening directory.'))
        ...
        page.add_row(Gtk.Label(label='Number of "steps" to take before flipping the page:'),
                     self._create_pref_spinner('number of key presses before page turn',
                                               1, 1, 100, 1, 3, 0,
                                               'Set the number of "steps" needed to flip to the next or previous page.'
                                               + ' Less steps will allow for very fast page turning but you'
                                               + ' might find yourself accidentally turning pages.'))
        ...
        page.add_row(Gtk.Label(label='Show only one page where appropriate:'), self._create_doublepage_as_one_control())
        ...
        if constants.PORTABLE_MODE:
            page.add_row(self._create_pref_check_button(
                    'Accept file(s) from anywhere',
                    'portable allow abspath',
                    'Accept file(s) from anywhere to be seved in bookmark and library.'
                    + ' If not, only files(s) in the same disk with mcomix will be accepted.'))
    ...
    def _init_display_tab(self):
        ...
        page.add_row(self._create_pref_check_button('Use fullscreen by default', 'default fullscreen', None))

        page.add_row(
                self._create_pref_check_button('Automatically hide all toolbars in fullscreen',
                                               'hide all in fullscreen',
                                               None))

        page.new_section('Fit to size mode')

        page.add_row(Gtk.Label(label='Fit to width or height:'), self._create_fitmode_control())

        page.add_row(Gtk.Label(label='Fixed size for this mode:'),
                     self._create_pref_spinner('fit to size px', 1, 10, 10000, 10, 50, 0, None))

        page.new_section('Slideshow')

        page.add_row(Gtk.Label(label='Slideshow delay (in seconds):'),
                     self._create_pref_spinner('slideshow delay', 1000.0, 0.01, 3600.0, 0.1, 1, 2, None))
        ...
        page.add_row(Gtk.Label(label='Slideshow step (in pixels):'),
                     self._create_pref_spinner('number of pixels to scroll per slideshow event',
                                               1, -500, 500, 1, 1, 0,
                                               'Specify the number of pixels to scroll while in slideshow mode.'
                                               + ' A positive value will scroll forward, a negative value will'
                                               + ' scroll backwards, and a value of 0 will cause the slideshow'
                                               + ' to always flip to a new page.'))
        ...
        page.add_row(Gtk.Label(label='Scaling mode'), self._create_scaling_quality_combobox())

        page.new_section('Advanced filters')

        page.add_row(Gtk.Label(label='Apply a high-quality scaling filter (main viewing area only):'),
    ...
    def _init_animation_tab(self):
        ...
        page.add_row(Gtk.Label(label='Animation mode'), self._create_animation_mode_combobox())
    ...
    def _init_advanced_tab(self):
        ...
        page.add_row(Gtk.Label(label='Sort files and directories by:'), self._create_sort_by_control())

        page.add_row(Gtk.Label(label='Sort archives by:'), self._create_archive_sort_by_control())
        ...
        page.add_row(
                Gtk.Label(label='Maximum number of concurrent extraction threads:'),
                self._create_pref_spinner(
        ...
        page.add_row(self._create_pref_check_button(
                ...
                'create thumbnails',
                'Store thumbnails for opened files according to the freedesktop.org specification.'
                + ' These thumbnails are shared by many other applications, such as most file managers.'))
        ...
        page.add_row(Gtk.Label(label='Maximum number of pages to store in the cache:'),
                     self._create_pref_spinner('max pages to cache',
                                               1, -1, 500, 1, 3, 0,
                                               'Set the max number of pages to cache. A value of -1 will'
                                               + ' cache the entire archive.'))

        if sys.platform == 'linux':
            page.add_row(self._create_pref_check_button(
                    'Mount tar and squashfs.',
                    'mount',
                    'Mount tar and squashfs by using archivemount or squashfuse.'))
        ...
        page.add_row(Gtk.Label(label='Magnifying lens size (in pixels):'),
                     self._create_pref_spinner('lens size',
                                               1, 50, 400, 1, 10, 0,
                                               'Set the size of the magnifying lens. It is a'
                                               + ' square with a side of this many pixels.'))
        ...
        page.add_row(Gtk.Label(label='Comment extensions:'), self._create_extensions_entry())
    ...
    def _create_language_control(self):
        ...
        autolang = [('Auto-detect (Default)', 'auto')]
        box = self._create_combobox(autolang + languages, prefs['language'], self._language_changed_cb)
    ...
    def _language_changed_cb(combobox, *args):
        ...
        if model_index > -1:
            itr = combobox.get_model().iter_nth_child(None, model_index)
            text, lang_code = combobox.get_model().get(itr, 0, 1)
    ...
    def _double_page_changed_cb(self, combobox, *args):
        ...
        if combobox.get_model().iter_is_valid(itr):
            value = combobox.get_model().get_value(itr, 1)
    ...
    def _create_fitmode_control(self):
        ...
        box = self._create_combobox(items, prefs['fit to size mode'], self._fit_to_size_changed_cb)
    ...
    def _create_sort_by_control(self):
        ...
        sortkey_items = (
                ...
                ('File name (locale aware)', constants.SORT_LOCALE))

        sortkey_box = self._create_combobox(sortkey_items, prefs['sort by'], self._sort_by_changed_cb)
    ...
    def _store_recent_changed_cb(self, combobox, *args):
        ...
        itr = combobox.get_active_iter()
        if not combobox.get_model().iter_is_valid(itr):
            return
    ...
    def _create_scaling_quality_combobox(self):
        ...
        box.set_tooltip_text('Changes how images are scaled. Slower algorithms result in'
                             + ' higher quality resizing, but longer page loading times.')
    ...
    def _create_pil_scaling_filter_combobox(self):
        ...
        box.set_tooltip_text(
                'If set, a scaling filter from PIL will be used instead of the "scaling quality"'
                + ' selection in the main view only. This will impact performance and memory usage.')
    ...
    def _create_combobox(options, selected_value, change_callback):
        ...
        while itr:
            if model.get_value(itr, 1) == selected_value:
                box.set_active_iter(itr)
                break
            else:
